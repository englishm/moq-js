...
async function testOutstandingRequestOwnership() {
	...
	assert.equal(publisher.hasRequest(0n), true)
	assert.equal(subscriber.hasRequest(0n), false)
	...
	assert.equal(publisher.hasRequest(0n), false)

	await withoutConsoleLog(() => subscriber.subscribe(["sub"], "track"))
	assert.equal(subscriber.hasRequest(2n), true)
	assert.equal(publisher.hasRequest(2n), false)
